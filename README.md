...
- **Task change log** — a faint 🕒 right after each task's title (List + Board) whose hover tooltip shows the **5 most recent** user edits, newest-first (`Trạng thái: To do → Done · 2h trước`). Records edits to title / status / priority / assignee / start / due / effort; the scheduler's auto-recomputed dates are deliberately **not** logged (the log captures *what you did*, not derived churn). Title keystroke-bursts coalesce into one entry; assignee names are frozen so history survives a member being deleted. Stored on the task (non-indexed, no schema bump). See [`design-docs/task-change-log.md`](./design-docs/task-change-log.md)
