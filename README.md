...
- **Archive sprint** — a reversible "hide out of the way" (not delete) to declutter a long list. Hover a sprint row → an **archive icon** at the right edge drops it into a collapsible **`Archived (N)`** section at the bottom (collapsed by default, hidden when empty); hover an archived row to **unarchive**. Archived sprints leave the active flow — the new-sprint default, `Sprint N` numbering, rollover target, and auto-select all consider non-archived only (archiving the current sprint hands off to the latest active one, never a blank view). Logged in the activity log. See [`design-docs/sprint-archive.md`](./design-docs/sprint-archive.md)
- **Three views**:
