...
import { STATUS_LABEL, PRIORITY_LABEL } from './lib'
...
/** First user-perceived character (grapheme-ish) of a name, for the initial. */
...
function StatusPill({ status }: { status: Status }) {
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, whiteSpace: 'nowrap' }}>
      ...
      <span style={{ color: C.muted, fontSize: 12 }}>{STATUS_LABEL[status]}</span>
    </span>
  )
}
...
export const PngExportCard = forwardRef<HTMLDivElement, PngExportCardProps>(
  function PngExportCard({ projectName, viewName, groups, today, width = 720 }, ref) {
    ...
    return (
      ...
      >
        ...
        <div style={{ height: 1, background: C.hair, margin: '18px 0 4px' }} />
        ...
        {groups.length === 0 ? (
          ...
          groups.map((g, gi) => {
            ...
            return (
              <div key={g.member?.id ?? '__unassigned'} style={{ marginTop: gi === 0 ? 14 : 20 }}>
                {/* Section header */}
                      ...
                      >
                        ...
                        <span
                          style={{
                            ...
                            flexShrink: 0,
                            fontVariantNumeric: 'tabular-nums',
                            color: overdue ? C.overdue : C.faint,
