...
describe('export / import round-trip', () => {
  ...
  it('clamps an oversized changeLog on import', async () => {
    const bloated: ChangeLogEntry[] = Array.from({ length: 12 }, (_, i) => ({
      field: 'status',
      from: 'todo',
      to: 'done',
      ts: i,
    ...
    await importAll({
      version: 2 as const,
      exportedAt: '',
      projects: [{ id: P, name: 'Test', createdAt: 0 }],
      members: [],
      sprints: [{ id: 's1', projectId: P, name: 'S', startDate: '2026-05-01', endDate: '2026-05-14' }],
      tasks: [
        {
    ...
    const t = await db.tasks.get('t1')
    expect(t?.changeLog?.length).toBe(5)
  })
})

describe('change log', () => {
  const SP = 's-cl'
  async function makeTask(over: Partial<Task> = {}): Promise<string> {
    const id = uid()
    await db.tasks.add({
      id,
      projectId: P,
  ...
  const logOf = async (id: string) => (await db.tasks.get(id))!.changeLog ?? []

  describe('appendChangeLog (pure)', () => {
    const E = (over: Partial<ChangeLogEntry>): ChangeLogEntry => ({
      field: 'status',
      from: 'todo',
      to: 'done',
      ts: 1000,
    ...
    it('prepends newest-first and caps at 5', () => {
      let log: ChangeLogEntry[] = []
      for (let i = 0; i < 7; i++) {
        log = appendChangeLog({ changeLog: log }, [
          E({ field: 'priority', from: String(i), to: String(i + 1), ts: i }),
        ])
      }
      expect(log).toHaveLength(5)
      expect(log[0].to).toBe('7') // newest
      expect(log[4].to).toBe('3') // oldest kept (0,1,2 dropped)
    })

    it('coalesces consecutive title edits within the window', () => {
      let log = appendChangeLog({ changeLog: [] }, [E({ field: 'title', from: '', to: 'A', ts: 0 })])
      log = appendChangeLog({ changeLog: log }, [E({ field: 'title', from: 'A', to: 'AP', ts: 1000 })])
      log = appendChangeLog({ changeLog: log }, [E({ field: 'title', from: 'AP', to: 'API', ts: 2000 })])
      expect(log).toHaveLength(1)
      expect(log[0]).toMatchObject({ field: 'title', from: '', to: 'API' })
    })

    it('drops the entry when a title coalesces back to its origin', () => {
      let log = appendChangeLog({ changeLog: [] }, [E({ field: 'title', from: 'A', to: 'AB', ts: 0 })])
      log = appendChangeLog({ changeLog: log }, [E({ field: 'title', from: 'AB', to: 'A', ts: 500 })])
      expect(log).toHaveLength(0)
    })

    it('does NOT coalesce title edits past the 2-min window', () => {
      let log = appendChangeLog({ changeLog: [] }, [E({ field: 'title', from: '', to: 'A', ts: 0 })])
      log = appendChangeLog({ changeLog: log }, [
        E({ field: 'title', from: 'A', to: 'B', ts: 2 * 60 * 1000 + 1 }),
      ])
      expect(log).toHaveLength(2)
    })

    it('never coalesces status — preserves distinct transitions', () => {
      let log = appendChangeLog({ changeLog: [] }, [E({ from: 'todo', to: 'in_progress', ts: 0 })])
      log = appendChangeLog({ changeLog: log }, [E({ from: 'in_progress', to: 'done', ts: 1000 })])
      expect(log).toHaveLength(2)
      expect(log[0]).toMatchObject({ from: 'in_progress', to: 'done' })
      expect(log[1]).toMatchObject({ from: 'todo', to: 'in_progress' })
    })
  })

  describe('updateTask', () => {
    it('logs a single changed loggable field with raw values', async () => {
      const id = await makeTask()
      await updateTask(id, { status: 'in_progress' })
      const log = await logOf(id)
      expect(log).toHaveLength(1)
      expect(log[0]).toMatchObject({ field: 'status', from: 'todo', to: 'in_progress' })
      expect((await db.tasks.get(id))!.status).toBe('in_progress')
    })

    it('logs one entry per changed field in a multi-field patch', async () => {
      const id = await makeTask()
      await updateTask(id, { status: 'done', priority: 'high' })
      const fields = (await logOf(id)).map((e) => e.field).sort()
      expect(fields).toEqual(['priority', 'status'])
    })

    it('ignores non-loggable keys (boardOrder) — powers the Board path', async () => {
      const id = await makeTask()
      await updateTask(id, { boardOrder: 3 })
      expect(await logOf(id)).toHaveLength(0)
      expect((await db.tasks.get(id))!.boardOrder).toBe(3)
    })

    it('skips a no-op write (same value)', async () => {
      const id = await makeTask({ status: 'todo' })
      await updateTask(id, { status: 'todo' })
      expect(await logOf(id)).toHaveLength(0)
    })

    it('freezes the assignee NAME and survives the member being deleted', async () => {
      await db.members.add({ id: 'm9', projectId: P, name: 'Bình', color: '#000', daysOff: [] })
      const id = await makeTask()
      await updateTask(id, { assigneeId: 'm9' })
      await db.members.delete('m9')
      const log = await logOf(id)
      expect(log[0]).toMatchObject({ field: 'assigneeId', from: null, to: 'Bình' })
    })

    it('treats an undefined changeLog (legacy row) as empty', async () => {
      const id = await makeTask()
    ...
    it('does NOT recompute dates (caller owns that)', async () => {
      const id = await makeTask({ startDate: '2026-05-01', dueDate: '2026-05-02' })
      await updateTask(id, { estimate: 3 })
      const t = await db.tasks.get(id)
      expect(t!.startDate).toBe('2026-05-01')
      expect(t!.dueDate).toBe('2026-05-02')
    })
  })

  describe('logStatusChange', () => {
    it('writes the status and logs the transition', async () => {
      const id = await makeTask()
      await logStatusChange(id, 'todo', 'done')
      const t = await db.tasks.get(id)
      expect(t!.status).toBe('done')
      expect(t!.changeLog![0]).toMatchObject({ field: 'status', from: 'todo', to: 'done' })
    })

    it('is a no-op when from === to', async () => {
      const id = await makeTask()
      await logStatusChange(id, 'todo', 'todo')
      expect(await logOf(id)).toHaveLength(0)
    })
  })
  ...
  describe('scheduler isolation (premise #2 — CRITICAL regression)', () => {
    it('recomputeDates shifting a downstream date adds NO changeLog entry', async () => {
      ...
      await makeTask({ id: 'a', estimate: 2, startDate: '2026-05-04' })
      await makeTask({ id: 'b', dependsOn: ['a'], estimate: 1 })
      await recomputeDates('a')
      const bBefore = (await db.tasks.get('b'))!
      // Now change a's effort directly (raw) and recompute — b's dates move.
      await db.tasks.update('a', { estimate: 6 })
      await recomputeDates('a')
      const bAfter = (await db.tasks.get('b'))!
      expect(bAfter.dueDate).not.toBe(bBefore.dueDate) // dates really moved
      expect(bAfter.changeLog ?? []).toHaveLength(0) // but nothing logged
    })

    it('recomputeAllDates never writes a changeLog entry', async () => {
