...
export interface ExportPayload {
   ...
   */
  shares?: ShareRecord[]
  /** v7 introduces per-project AI Chat history. */
  aiThreads?: AiThread[]
  aiMessages?: AiMessage[]
}

export async function exportAll(): Promise<ExportPayload> {
  const [projects, members, sprints, collections, tasks, events, people, shares, aiThreads, aiMessages] =
    await Promise.all([
      db.projects.toArray(),
      db.members.toArray(),
      db.sprints.toArray(),
...
export async function importAll(data: ExportPayload) {
  if (!data || ![1, 2, 3, 4, 5, 6, 7].includes(data.version)) {
  ...
  try {
   await db.transaction(
    ...
    async () => {
      ...
      if (Array.isArray(data.aiThreads) && data.aiThreads.length) {
        await db.aiThreads.bulkAdd(
          data.aiThreads.map((t) => ({ ...t, projectId: pidOf(t) }))
        )
      }
      if (Array.isArray(data.aiMessages) && data.aiMessages.length) {
