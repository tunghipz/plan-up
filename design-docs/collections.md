...
**Status:** Implemented
**Last updated:** 2026-06-29 (Collection list rows show item title + member + duration/effort)
...
`app/src/CollectionView.tsx` (List card-per-section + status editor + item title +
Member assignment + Duration/Effort editing),
...
> Nếu user muốn một backlog, họ tạo collection thường tên "Backlog"; app không còn auto-create
> hoặc bảo vệ Backlog như system collection.
...
- Cột: **Name · Member · Duration · Sprint · Actions**. Collection List expose member
  assignment, duration/effort, and a direct **Add** menu to move an item into a sprint;
  date/status planning metadata không hiện trong List để collection bớt nhiễu. Actions có
  xoá item với inline confirm.
...
metadata nhẹ (`assigneeId`, `estimate`, `startDate`, `dueDate`) nếu được set từ tool/import hoặc
flow khác. Collection List hiện cho chỉnh `assigneeId` như Member và `estimate` như Duration;
trạng thái calendar/status vẫn dùng `collectionStatusId` khi dữ liệu có sẵn.
...
- **`CollectionView.tsx`** — view (`list`/`calendar`) là **controlled prop** do `App.tsx` truyền
  ...
  list-view của sprint** nhưng tối giản columns: lead grip-gutter · title flex-1 · Member `w-32` ·
  Duration `w-24` · Sprint `w-36` · Actions. Cùng column-header `bg-canvas-sunk/40` + nhãn `text-[11px] text-ink-faint`,
  các row trong `divide-y divide-border`. **Title luôn ở chế độ sửa**
  ...
  **Add item inline** (gõ + Enter ngay trong card, không dùng `window.prompt`). Kéo-thả item giữa
  ...
  nhật `sectionId`. Member picker ghi vào `Task.assigneeId`; Duration dùng chung `EffortCell`
  với sprint List và ghi vào `Task.estimate`. Sprint menu suggest sprint hiện tại + 1-2
  sprint kế tiếp và gọi `moveTaskToSprint`.
...
- Tái dùng: native assignee select + `EffortCell` cho Member/Duration trong List, list card +
  drag-reorder; Calendar vẫn dùng date/status rendering riêng theo collection status.
...
- **AI collection tools:** AI Chat có thể propose `create_collection`,
  ...
  đang chọn. Collection tên `Backlog` không có ngoại lệ system; nó rename/delete
  như mọi collection khác.
- **Collection task tools:** AI Chat có thể propose `update_task` và `delete_task`
  trên task đang visible trong collection. Trường assign dùng `assigneeName`;
  xoá dùng `delete_task` và vẫn cần user apply trong chat drawer.
