...
- Shell in `app/src-tauri/` (project rule: all code under `app/`): `tauri.conf.json`,
  `Cargo.toml`, `src/lib.rs` (plugins + the four backup commands: `write_backup`,
  `prune_backups`, `list_backups`, `read_backup`), `src/main.rs`,
...
- Capabilities: only `core:default` + `dialog:allow-open`. **No fs plugin** — file
  writes go through those custom Rust commands (see auto-backup.md security model).
- Only non-Tauri Rust dep: `flate2`, for gzipping the `versions/` backup tier
  (auto-backup.md, *Disk footprint*).
