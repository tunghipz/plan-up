...
> **Brand theme (2026-08-17):** app **pin cứng Cupertino Blue** `#0071E3`. Theme
> **ZingPlay Fire** vẫn còn trong `index.css` (`[data-brand="fire"]`) nhưng **không phải
> default và không có UI chọn** — nút Flame ở footer sidebar đã ẩn từ 2026-07-15, và
> 2026-08-17 hook `useBrandTheme` đổi default Fire → Blue (bỏ qua `plan-up:brand` cũ).
...
KHÔNG mix về `#000` (làm giảm contrast ở dark mode).
...
**Amendments 2026-07-07 (audit pass):**
- **Utility fade ≤150ms được sanction**: Tailwind `transition`/`transition-colors` mặc định (150ms, easing default) cho hover/focus color-opacity fades là hợp lệ — không cần đổi sang 2 curve chung (rẻ, không cảm nhận được easing ở 150ms).
- Mọi animation/transition **thấy được chuyển động** (slide, scale, pulse, rise) phải nằm trong guard `prefers-reduced-motion: reduce` hoặc mang class `motion-reduce:*`. Color/opacity fade thuần được miễn.
- `task-flash` rút 1.25s → **0.6s**; `status-pop` 0.32s → 0.3s. Không còn ngoại lệ ngoài capacity 0.35s + act-enter 0.34s.
- Hover transform trang trí (scale/translate) phải kèm `motion-reduce:transform-none`.

**Overlay keyboard contract (2026-07-07):** mọi overlay (ModalSheet, ConfirmDialog, drawer, popover, menu) phải đóng được bằng **Esc** và outside-click; Esc chỉ đóng **tầng trên cùng** (`stopPropagation` theo tầng); shortcut phím đơn (`n`, `/`) không fire khi có modal đang mở; ConfirmDialog destructive **autoFocus vào Cancel** (không phải nút đỏ); ModalSheet mang `role="dialog"` + `aria-modal` + nút Done/Close nhìn thấy được.
